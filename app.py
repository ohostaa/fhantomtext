    ...
    function draw() {
        ...
        lines.forEach((line, lineIdx) => {
            ...
            for (let char of line) {
                ...
                // ★ここが修正ポイント: 文字本体（Fill）の描画
                // 白ベタ(#FFFFFF)ではなく、「下から色、上が白」になるような縦グラデーションを作成
                // ベース色は、上で計算した colorStr (その文字の位置の色) を使う
                
                // グラデーション座標: 下(baseY+size/2) から 上(baseY-size/2)
                const gradTop = baseY + yOffset - fontSize * 0.6;
                const gradBottom = baseY + yOffset + fontSize * 0.6;
                const fillGrad = ctx.createLinearGradient(0, gradBottom, 0, gradTop);
                
                // 下: 濃い色 (colorStr)
                fillGrad.addColorStop(0.0, colorStr);
                // 中間: 少し明るく
                fillGrad.addColorStop(0.4, colorStr);
                // 上: 白 (発光して輝いている感じ)
                fillGrad.addColorStop(1.0, "#FFFFFF");

                ctx.shadowBlur = 0; // 本体の輪郭はクッキリさせるためBlurなし
                ctx.globalAlpha = 1.0;
                ctx.fillStyle = fillGrad; // 作成したグラデーションで塗る
                ctx.fillText(char, currentX, baseY + yOffset);
